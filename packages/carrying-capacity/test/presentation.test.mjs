...
test('public presentation contains perennial evidence, reference-only candidates and metric display units', () => {
  ...
  assert.equal(contract.agroecosystem.contract_version, '1.3.0');
  ...
  assert.equal(row24.total_parcel_hectares, 23.21512);
  ...
  assert.equal(acquisitionCrossover.first_over_20_ha.adult_residents, 22);
  assert.equal(acquisitionCrossover.market_band_crossover.adult_residents, 22);
  assert.equal(acquisitionCrossover.economic_sweet_spot.next_adult_residents, 24);
  assert.equal(acquisitionCrossover.market_band_crossings.find((row) => row.adult_residents === 42).new_land_band, '40+ ha');
  assert.equal(acquisitionCrossover.market_band_crossings.find((row) => row.adult_residents === 42).evidence_sufficient_for_new_band, true);
