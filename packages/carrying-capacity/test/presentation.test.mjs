...
test('public presentation contains perennial evidence, reference-only candidates and metric display units', () => {
  ...
  assert.equal(row24.total_parcel_hectares, 21.876628);
  ...
  assert.equal(acquisitionCrossover.market_band_crossings.find((row) => row.adult_residents === 44).new_land_band, '40+ ha');
  assert.equal(acquisitionCrossover.market_band_crossings.find((row) => row.adult_residents === 44).evidence_sufficient_for_new_band, true);
