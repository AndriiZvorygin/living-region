...
import {calculateHouseholdFoodDemandProfile, householdLandRole} from '../src/household-demand.mjs';
...
export function buildHouseholdCapacity(energy = buildHealthCanadaEnergy(), food = calculateFoodEvidence(), heating = buildEvidenceHeating(), woody = calculateWoodyLand(heating)) {
  ...
  for (const [siteId, site] of Object.entries(siteClasses)) {
    ...
    for (const [householdId, household] of Object.entries(householdProfiles)) {
      ...
      const demandProfile = calculateHouseholdFoodDemandProfile(membersResult);
      ...
      const permanentAdultMembers = membersResult.filter((member) => householdLandRole(member) === 'permanent_adult');
      const permanentAdultFoodSystem = calculateFoodSystem(food, demandProfile.permanent_adult_food_demand_gj_year, site, permanentAdultMembers.reduce((sum, member) => sum + member.weight_kg, 0));
      const mathArea = foodSystem.required_food_area_ha + heatArea;
      const resilience = {diversity_and_rotation_ha: round(Math.max(.12, permanentAdultFoodSystem.required_food_area_ha * .25), 6), soil_water_perennial_buffer_ha: .15, fibre_habitat_wildlife_buffer_ha: .10, deliberate_export_production_ha: .20};
      ...
      const arcAllocationHa = demandProfile.permanent_adult_count * arcPolicyAdultAllocationHa;
