...
|---|---|---|
| `data/source/food-production-labour.csv` | labour tasks, establishment versus recurring hours, annual soil preparation, planting, weeding, irrigation/monitoring, harvest, pruning, orchard maintenance, pest/wildlife work, processing/storage, greenhouse management and coppice/firewood tasks | evidence-informed planning classification; hours are not a time-and-motion study; woody task hours are explicit ARC planning assumptions |
...
The mature land-accounting audit is generated in `outputs/land-accounting-audit.md`. It maps the former resilience allowance to 0.12 ha diversity/rotation reserve, 0.15 ha soil/water functional coverage, 0.10 ha fibre/habitat/wildlife functional coverage and 0.20 ha optional export target. Only the reserve is retained as an exclusive central robust allowance; ecological functions overlap perennial/woody production and market production first uses released annual ground. Paths/access and greenhouse/building footprints are not allocated and require parcel-level design.
