...
import {AGRO_MACRO_TARGET_RANGES, calculateAgroecosystemPlan} from './agroecosystem.mjs';
import {FOOD_PRODUCTION_LABOUR_CONTRACT_VERSION, LABOUR_CAPACITY_THRESHOLDS, LABOUR_PROJECTION_MODE, LABOUR_MECHANIZATION_ASSUMPTION, LABOUR_CATEGORIES, CLOSED_LOOP_WORK_ITEMS, LABOUR_PROFILE_BY_PRODUCTION_ID} from './production-labour.mjs';
...
export function buildCarryingCapacityPresentationContract({produceDir = 'know/produce', generatedAt = new Date().toISOString()} = {}) {
  ...
  return {
    ...
    agroecosystem: {contract_version: AGROECOSYSTEM_CONTRACT_VERSION, plant_database_version: PLANT_DATABASE_VERSION, plant_database_contract_version: PLANT_DATABASE_CONTRACT_VERSION, labour_contract_version: FOOD_PRODUCTION_LABOUR_CONTRACT_VERSION, labour_projection_mode: LABOUR_PROJECTION_MODE, labour_mechanization: publicize(LABOUR_MECHANIZATION_ASSUMPTION), labour_categories: LABOUR_CATEGORIES, closed_loop_work_items: publicize(CLOSED_LOOP_WORK_ITEMS), labour_capacity_thresholds: LABOUR_CAPACITY_THRESHOLDS, labour_profile_map: LABOUR_PROFILE_BY_PRODUCTION_ID, macro_target_ranges: publicize(AGRO_MACRO_TARGET_RANGES), macro_energy_method: 'Protein and carbohydrate use 0.016736 GJ/kg and fat uses 0.037656 GJ/kg; fibre is not an independent energy source.', source_manifest: plantSource.source_manifest.map(publicText), records: plantDatabase.records.map((record) => publicize({id: record.id, identity: record.identity, architecture: record.architecture, establishment: record.establishment, site_needs: record.site_needs, outputs: record.outputs, ecological_function: record.ecological_function, management: record.management, relationships: record.relationships, evidence: record.evidence})), reference_plan: publicAgroecosystem(agroecosystemReference), objectives: Object.fromEntries(Object.entries(AGROECOSYSTEM_OBJECTIVES).map(([id, row]) => [id, row.label])), model_boundary: 'This planner selects compatible plants, schedules annual plots, reconciles nutrient stocks and reports current-ration versus optimizer macro status. Existing household energy, nutrition, livestock and land APIs remain the downstream canonical reporting interfaces during migration.'},
