...
export function calculateInteractiveHousehold(options = {}) {
  ...
  const proteinDemand = calculateHouseholdProteinDemand(members);
  const nutrientFood = calculateNutrientFoodSystem({foodEvidence: options.foodEvidence, demandGJ: result.household_food_gj_year, proteinDemandKgYear: proteinDemand.household_protein_kg_year, members, siteCapability: {...site, calculateFoodSystem}, livestockMode: options.livestockMode ?? 'plants_only', rationId: options.livestockRation ?? 'arc_integrated', livestockScale: options.livestockScale ?? 1, establishmentYears: options.establishmentModel?.years});
