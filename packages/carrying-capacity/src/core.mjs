...
export const siteClasses = {wetter_productive: {label: 'Wetter productive site', food_multiplier: 1, woody_band: 'favourable', notes: 'Moisture-tolerant woody systems can be considered, but exceptional saturated-site trials are excluded.'}, ordinary_mesic: {label: 'Ordinary mesic site', food_multiplier: 1, woody_band: 'ordinary', notes: 'Central planning class for reasonably designed mixed perennial and annual production.'}, dry: {label: 'Dry site', food_multiplier: .75, woody_band: 'marginal', notes: 'Water limitation is represented as a transparent scenario multiplier, not a measured Grey County average.'}, shallow_rocky_marginal: {label: 'Shallow/rocky marginal site', food_multiplier: .5, woody_band: 'marginal', notes: 'Lower productivity scenario; a site survey is required before using it for a parcel.'}};
...
export function calculateFoodSystem(foodEvidence, demandGJ, siteMultiplier = 1, proteinReferenceWeightKg = 70) {
  ...
  const rows = Object.entries(wanted).map(([id, energyShare]) => { const row = foodEvidence.rows.find((item) => item.id === id); if (!row || !row.food_gj_ha) throw new Error(`Missing canonical food-system row: ${id}`); const foodGJHa = row.food_gj_ha * siteMultiplier; return {id, crop: row.crop, category: row.category, energy_share: energyShare, food_gj_ha: foodGJHa, area_ha: demandGJ * energyShare / foodGJHa, protein_kg_ha: row.protein_kg_ha * siteMultiplier, fat_kg_ha: row.fat_kg_ha * siteMultiplier, carbohydrate_kg_ha: row.carbohydrate_kg_ha * siteMultiplier}; });
...
export function calculateInteractiveHousehold({members = [], buildings = [defaultBuilding()], siteId = 'ordinary_mesic', foodEvidence, woodyCases, matureReferenceRow} = {}) {
  const demand = members.reduce((sum, member) => sum + Number(member.gj_year), 0); const referenceWeight = members.reduce((sum, member) => sum + Number(member.weight_kg), 0); const site = siteClasses[siteId] ?? siteClasses.ordinary_mesic; const food = calculateFoodSystem(foodEvidence, demand, site.food_multiplier, referenceWeight); const heating = calculateHeatingLoads({buildings}); const woodBand = woodyCases?.central?.[site.woody_band]; const woodYield = Number(woodBand?.usable_gross_energy_gj_ha_year ?? woodBand?.usable_gross_energy_gj_year ?? 0); const heatingArea = woodYield > 0 ? heating.total_gross_wood_energy_gj_year / woodYield : 0; const resilience = {diversity_and_rotation_ha: round(Math.max(.12, food.required_food_area_ha * .25)), soil_water_perennial_buffer_ha: .15, fibre_habitat_wildlife_buffer_ha: .1}; const landAllocation = calculateExclusiveLandAllocation({foodAreaHa: food.required_food_area_ha, heatingAreaHa: heatingArea, reserveHa: resilience.diversity_and_rotation_ha}); const robustMinimum = landAllocation.exclusive_total_ha; const referenceDemand = Number(matureReferenceRow?.household_food_gj_year ?? demand) || demand; const referenceLabour = matureReferenceRow?.recurring_labour ?? {};
  const labourCapacity = calculateHouseholdLabourCapacity(members); const scale = demand / referenceDemand; const labourRequired = {hours_year: Number(referenceLabour.total_recurring_labour_hours ?? 0) * scale, heavy_hours_year: Number(referenceLabour.physically_demanding_hours ?? 0) * scale};
  return {members, buildings, household_food_gj_year: round(demand), food_adult_equivalents: round(demand / FOOD_ADULT_EQUIVALENT_GJ_YEAR), food_adult_equivalent_basis_gj_year: round(FOOD_ADULT_EQUIVALENT_GJ_YEAR), food_area_ha: food.required_food_area_ha, heating_area_ha: round(heatingArea), heating, resilience_allowances_ha: resilience, land_allocation: {...landAllocation, exclusive_total_ha: round(landAllocation.exclusive_total_ha)}, robust_minimum_area_ha: round(robustMinimum), reference_transition_scale: round(scale), labour: {required_hours_year: round(labourRequired.hours_year), required_heavy_hours_year: round(labourRequired.heavy_hours_year), available_hours_year: round(labourCapacity.available_hours_year), available_heavy_hours_year: round(labourCapacity.heavy_work_hours_year), surplus_hours_year: round(labourCapacity.available_hours_year - labourRequired.hours_year), surplus_heavy_hours_year: round(labourCapacity.heavy_work_hours_year - labourRequired.heavy_hours_year), capacity: labourCapacity}, site_id: siteId, caveat: 'Food-adult-equivalent is a food-energy normalization only, not a total-land multiplier. Food demand and assigned labour capacity are independent inputs; custom transition rows scale the canonical reference transition by Health Canada food energy.'};
}
