...
import {compareFoodLabourCapacity} from './production-labour.mjs';
...
export {FOOD_ADULT_EQUIVALENT_GJ_YEAR};
...
export function calculateExclusiveLandAllocation({foodAreaHa = 0, heatingAreaHa = 0, reserveHa = 0, feedAreaHa = 0, portfolioAreaHa = 0} = {}) {
  const parts = [{id: 'annual', label: 'Annual food', area_ha: foodAreaHa * .25}, {id: 'perennial', label: 'Perennial food', area_ha: foodAreaHa * .75}, ...(Number(portfolioAreaHa) > 0 ? [{id: 'portfolio', label: 'Additional portfolio production', area_ha: portfolioAreaHa}] : []), ...(Number(feedAreaHa) > 0 ? [{id: 'feed', label: 'Dedicated livestock feed', area_ha: feedAreaHa}] : []), {id: 'wood', label: 'Renewable heating biomass', area_ha: heatingAreaHa}, {id: 'reserve', label: 'Exclusive reserve', area_ha: reserveHa}];
...
export function calculateFoodSystem(foodEvidence, demandGJ, siteMultiplierOrCapability = 1, proteinReferenceWeightKg = 70) {
  ...
  const rows = viable.map(([id, baseEnergyShare]) => { const energyShare = baseEnergyShare / viableWeight; const row = foodEvidence.rows.find((item) => item.id === id); if (!row || !row.food_gj_ha) throw new Error(`Missing canonical food-system row: ${id}`); const cropRule = cropRules[id]; const cropMultiplier = cropRule && Number.isFinite(Number(cropRule.yield_multiplier)) ? Number(cropRule.yield_multiplier) : siteMultiplier; const foodGJHa = row.food_gj_ha * cropMultiplier; return {id, crop: row.crop, category: row.category, composition_id: row.composition_id ?? null, energy_share: energyShare, viability: 'viable', site_yield_multiplier: cropMultiplier, food_gj_ha: foodGJHa, edible_yield_t_ha: Number(row.edible_yield_t_ha ?? 0) * cropMultiplier, area_ha: demandGJ * energyShare / foodGJHa, protein_kg_ha: row.protein_kg_ha * cropMultiplier, fat_kg_ha: row.fat_kg_ha * cropMultiplier, carbohydrate_kg_ha: row.carbohydrate_kg_ha * cropMultiplier}; });
...
function calculateInteractiveHouseholdLegacy({members = [], buildings = [defaultBuilding()], siteId = 'ordinary_mesic', foodEvidence, woodyCases, matureReferenceRow, establishmentModel = null, arcPolicyAllocationHa = null, livestockMode = 'plants_only', livestockRation = 'arc_integrated', livestockScale, nutritionGoal = 'user_selected_animal_share'} = {}) {
  ...
  const nutrientFood = calculateNutrientFoodSystem({foodEvidence, demandGJ: demand, demandByYear: demandProfile.demand_by_year, proteinDemandKgYear: proteinDemand.household_protein_kg_year, members, siteCapability: {...site, calculateFoodSystem}, livestockMode, rationId: livestockRation, livestockScale, nutritionGoal, establishmentYears: establishmentModel?.years, perennialMix: establishmentModel?.perennial_mix ?? [], curveAnchors: establishmentModel?.curve_anchors ?? {}, permanentAdultDemandGJ: demandProfile.permanent_adult_food_demand_gj_year, annualIntercropOverlapByYear: establishmentModel?.annual_intercrop_overlap_by_year});
  ...
  const portfolioArea = Number(nutrientFood.portfolio_land?.additional_area_ha ?? 0);
  const landAllocation = calculateExclusiveLandAllocation({foodAreaHa: food.required_food_area_ha, heatingAreaHa: heatingArea, reserveHa: resilience.diversity_and_rotation_ha, feedAreaHa: nutrientFood.feed.additional_dedicated_feed_land_ha, portfolioAreaHa: portfolioArea});
  ...
  const foodProductionLabour = nutrientFood.food_production_labour ?? null;
  const participatingWorkers = labourCapacity.members.filter((member) => Number(member.available_hours_year) > 0).length;
  const householdFoodProductionLabour = foodProductionLabour ? {...foodProductionLabour, stages: foodProductionLabour.stages.map((stage) => ({...stage, hours_per_person_week: participatingWorkers > 0 ? round(stage.total_hours_year / (365.25 / 7) / participatingWorkers, 2) : null}))} : null;
  const foodLabourCapacity = householdFoodProductionLabour ? compareFoodLabourCapacity(householdFoodProductionLabour, {availableHoursYear: labourCapacity.available_hours_year, participatingWorkers}) : null;
  return {members, buildings, household_food_gj_year: round(demand), permanent_adult_food_demand_gj_year: round(demandProfile.permanent_adult_food_demand_gj_year), dependent_child_food_demand_gj_year: round(demandProfile.dependent_child_food_demand_gj_year), food_demand_profile: demandProfile, food_adult_equivalents: round(demand / FOOD_ADULT_EQUIVALENT_GJ_YEAR), food_adult_equivalent_basis_gj_year: round(FOOD_ADULT_EQUIVALENT_GJ_YEAR), adult_equivalent_scope: 'food-energy normalization only; not a total-land multiplier', food, food_area_ha: food.required_food_area_ha, portfolio_food_area_ha: round(nutrientFood.portfolio_land?.total_food_area_with_portfolio_ha ?? food.required_food_area_ha), portfolio_additional_area_ha: round(portfolioArea), heating_area_ha: round(heatingArea), heating, resilience_allowances_ha: resilience, land_allocation: {...landAllocation, exclusive_total_ha: round(landAllocation.exclusive_total_ha)}, robust_minimum_area_ha: round(robustMinimum), establishment_land: establishmentLand, arc_policy_allocation_ha: round(policyAllocation), reference_transition_scale: round(scale), nutrient_food_system: nutrientFood, nutrition_goal: nutritionGoal, food_production_labour: householdFoodProductionLabour, food_labour_capacity: foodLabourCapacity, labour: {required_hours_year: round(labourRequired.hours_year), required_heavy_hours_year: round(labourRequired.heavy_hours_year), available_hours_year: round(labourCapacity.available_hours_year), available_heavy_hours_year: round(labourCapacity.heavy_work_hours_year), surplus_hours_year: round(labourCapacity.available_hours_year - labourRequired.hours_year), surplus_heavy_hours_year: round(labourCapacity.heavy_work_hours_year - labourRequired.heavy_hours_year), capacity: labourCapacity}, site_id: siteId, caveat: 'Food-adult demand sizes the permanent perennial footprint; portfolio rows are assigned within existing food zones before any overflow is added. Food-adult equivalents are a food-energy normalization only. Dependent children increase pooled current food demand and annual bridge requirements only while they remain under the household land-adult age. Annual and perennial outputs are pooled, and former dependent children transition to their own ARC allocation.'};
}
...
export function calculateInteractiveHousehold(options = {}) {
  ...
  const proteinDemand = calculateHouseholdProteinDemand(members);
  const nutrientFood = result.nutrient_food_system ?? calculateNutrientFoodSystem({foodEvidence: options.foodEvidence, demandGJ: result.household_food_gj_year, proteinDemandKgYear: proteinDemand.household_protein_kg_year, members, siteCapability: {...site, calculateFoodSystem}, livestockMode: options.livestockMode ?? 'plants_only', rationId: options.livestockRation ?? 'arc_integrated', livestockScale: options.livestockScale, nutritionGoal: options.nutritionGoal ?? 'user_selected_animal_share', establishmentYears: options.establishmentModel?.years});
  return {...result, protein_demand: proteinDemand, nutrient_food_system: nutrientFood, nutrition_goal: options.nutritionGoal ?? 'user_selected_animal_share', livestock_mode: options.livestockMode ?? 'plants_only', livestock_ration: options.livestockRation ?? 'arc_integrated', livestock_feed_area_ha: nutrientFood.feed.additional_dedicated_feed_land_ha};
}
