...
function calculateInteractiveHouseholdLegacy({members = [], buildings = [defaultBuilding()], siteId = 'ordinary_mesic', foodEvidence, woodyCases, matureReferenceRow, establishmentModel = null, arcPolicyAllocationHa = null, livestockMode = 'plants_only', livestockRation = 'arc_integrated', livestockScale} = {}) {
  ...
  const nutrientFood = calculateNutrientFoodSystem({foodEvidence, demandGJ: demand, proteinDemandKgYear: proteinDemand.household_protein_kg_year, members, siteCapability: {...site, calculateFoodSystem}, livestockMode, rationId: livestockRation, livestockScale, establishmentYears: establishmentModel?.years});
...
export function calculateInteractiveHousehold(options = {}) {
  ...
  const proteinDemand = calculateHouseholdProteinDemand(members);
  const nutrientFood = result.nutrient_food_system ?? calculateNutrientFoodSystem({foodEvidence: options.foodEvidence, demandGJ: result.household_food_gj_year, proteinDemandKgYear: proteinDemand.household_protein_kg_year, members, siteCapability: {...site, calculateFoodSystem}, livestockMode: options.livestockMode ?? 'plants_only', rationId: options.livestockRation ?? 'arc_integrated', livestockScale: options.livestockScale, establishmentYears: options.establishmentModel?.years});
