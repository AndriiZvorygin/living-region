...
async function main() {
  ...
  const report = {
    ...
    active_roofs: {
      ...
      with_legacy_fallback_addresses: activeFeatures.filter((feature) => feature.properties.address_quality === "legacy_unverified").length,
      with_legacy_nar_confirmed_addresses: activeFeatures.filter((feature) => feature.properties.address_quality === "legacy_nar_confirmed").length,
