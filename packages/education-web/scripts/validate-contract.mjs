...
if (house.contract_version !== '2.0.0') throw new Error('house-cost contract must expose audited v2 accounting');
if (!house.service_package_accounting?.arc_household_systems?.water_plumbing_sanitation) throw new Error('house-cost contract is missing inclusive ARC package accounting');
if (!house.central.legacy_reconciliation?.bridge?.former_model_economic_capital_cad) throw new Error('house-cost contract is missing the former-model reconciliation bridge');
if (house.central.components.some((row) => row.id === 'sanitation_greywater' || row.id === 'water_plumbing')) throw new Error('house-cost contract still exposes obsolete split ARC utility rows');
