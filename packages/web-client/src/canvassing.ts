...
export async function canvassingMain() {
  ...
  accountTools.innerHTML = `<details class="account-menu"><summary><span id="authenticated-user">${escapeHtml(authenticatedUser.display_name)} · ${authenticatedUser.role === "volunteer" ? "Volunteer" : "Candidate"}</span></summary><div class="account-menu-panel">${authenticatedUser.role === "candidate" ? '<a class="account-link" href="/canvassing/admin/users">Users</a><a class="account-link" href="/canvassing/admin/lawn-signs">Lawn signs</a>' : ""}<button id="change-password" type="button">Change password</button><button id="logout" type="button">Log out</button></div></details>`;
  ...
  const setNextAreaStatus = (message: string) => {
    ...
    if (mobileButton) {
      ...
      mobileButton.textContent = "Next area";
