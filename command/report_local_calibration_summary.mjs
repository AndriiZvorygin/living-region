...
try {
  ...
  const result = buildLocalCalibrationSummary({
  ...
  if (result.status !== 'pass') {
    console.error('local calibration summary failed');
    for (const failure of result.failures) console.error(`- ${failure}`);
    process.exit(1);
  }
  console.log(`json: ${result.paths.jsonPath}`);
  console.log(`markdown: ${result.paths.mdPath}`);
  console.log(`data points: ${result.summary.totalDataPoints}`);
  for (const warning of result.warnings) console.warn(`warning: ${warning}`);
} catch (error) {
  console.error(`local calibration summary command failed: ${error.message}`);
  process.exit(1);
}
