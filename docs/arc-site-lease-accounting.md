...
2. **Site lease** recovers the entire project property through two layers:
   - the **common-property land holding share**, divided equally across households, covers the geometry-derived physical common-property/access pool's acquisition/debt and tax. The current prototype is the public-road-to-laneway-to-terminal-loop-to-250 m² amenity envelope geometry; parcel/fire/municipal validation is still required. Paid administration, vacancy, optional insurance and common-land operating cash are zero in legal-minimum and resident labour is shown separately;
...
The land-holding entity owns one property. Productive household allocations, roads/access land, common spaces, ecological buffers and shared-building land are all included in the property area. The legal-minimum default now uses the **geometry-derived common-area prototype** documented in `docs/arc-common-area-geometry.md`: a configurable 50 m physical laneway corridor, terminal circulation lane and central 250 m² common envelope. The former 1.50 ha pool remains an explicit legacy sensitivity, not a measured requirement. Common-property value is assigned to the equal common-property land holding pool, and the resulting site-lease revenue is checked against the annual land-layer cost before shared infrastructure revenue is included.
...
The API accepts explicit residential footprint, internal road/access, common-building, ecological/water-buffer, shared productive and other common-land areas, and also accepts the ARC geometry prototype. The prototype is connected to economics but remains a conceptual, unvalidated site takeoff. See `docs/arc-common-area-geometry.md`, `arc-legal-minimum.md` and `arc-common-property-audit.md` for the boundary, sensitivity and spatial accounting pathway.
