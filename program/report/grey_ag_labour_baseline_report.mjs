...
export function buildGreyAgLabourBaselineReport(options = {}) {
  ...
  const report = {
    ...
    occupationSourceStatus: agLabour?.occupationSourceStatus ?? 'missing',
    currentAgLabourPreferredBasis: agLabour?.currentAgLabourPreferredBasis ?? 'industryProxy',
    ...
    coreAgOccupationWorkers: n(agLabour.coreAgOccupationWorkers),
    minorGroupCoreAgProxyWorkers: n(agLabour.minorGroupCoreAgProxyWorkers),
    coreAgOccupationFTEEstimate: n(agLabour.coreAgOccupationFTEEstimate),
    agIndustryFTEEstimate: n(agLabour.agIndustryFTEEstimate),
    broadAdjacentFTEEstimate: n(agLabour.broadAdjacentFTEEstimate),
  ...
  const markdown = [
    ...
    `- occupationSourceStatus: ${report.occupationSourceStatus}`,
    `- currentAgLabourPreferredBasis: ${report.currentAgLabourPreferredBasis}`,
    ...
    `- coreAgOccupationWorkers: ${report.coreAgOccupationWorkers}`,
    `- minorGroupCoreAgProxyWorkers: ${report.minorGroupCoreAgProxyWorkers}`,
    ...
    '- if unit-group occupation rows are unavailable for Grey CD, the model uses 98-10-0594-01-style minor-group core-ag proxy before falling back to industry proxy',
    '',
    '## Sanity flags',
