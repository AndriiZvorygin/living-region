...
export function buildGreyTransitionPathwayReport(options = {}) {
  ...
  const carryingCapacityAdoption = calculateGreyCarryingCapacityAdoption({
    eligibleHouseholds: n(dwelling.estimatedDwellingsWithGardenScaleAccess),
    eligiblePopulation: n(dwelling.estimatedPopulationWithGardenScaleAccess),
    regionalFoodDemandGJ: totalDemandGJ,
    eligibilityBasis: 'estimatedDwellingsWithGardenScaleAccess and corresponding population from the current Grey dwelling-land proxy; not legal parcel access or biological site classification'
  });
  ...
  const md = [
    ...
    '',
    '## Canonical carrying-capacity adoption scenarios',
    'The adoption rows below are generated by the canonical carrying-capacity package. They keep the best current dwelling-land eligibility proxy, household composition mix, favourable/ordinary/marginal site shares, annual bridge, perennial transition, woody heating and labour separate from the broader food-price/stress scenario model.',
    '| Adoption | External-input condition | Mature households | Mature people | Establishment land (ha) | Mature land (ha) | Current footprint (ha) | ARC establishment comparison (ha) | Market demand displaced (GJ/year) | Exportable surplus (GJ/year) |',
    '|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|',
    ...carryingCapacityAdoption.scenarios.map((scenario) => { const row = scenario.transition_years.find((candidate) => candidate.year === 'mature'); return `| ${scenario.adoption_percent.toFixed(0)}% | ${scenario.external_input_condition} | ${row.participating_households.toFixed(0)} | ${row.participating_population_people.toFixed(0)} | ${row.establishment_land_requirement_ha.toFixed(1)} | ${row.mature_land_requirement_ha.toFixed(1)} | ${row.productive_land_required_ha.toFixed(1)} | ${row.arc_policy_establishment_surplus_or_deficit_ha.toFixed(1)} | ${row.market_food_demand_displaced_gj_year.toFixed(0)} | ${row.mature_exportable_surplus_gj_year.toFixed(0)} |`; }),
    '',
    '## Caveats',
