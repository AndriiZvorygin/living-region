...
export function buildGreyTransitionPathwayReport(options = {}) {
  ...
  const md = [
    ...
    '| Adoption | External-input condition | Mature participating households | Mature people | Productive land (ha) | Market demand displaced (GJ/year) | Exportable surplus (GJ/year) |',
    '|---:|---|---:|---:|---:|---:|---:|',
    ...carryingCapacityAdoption.scenarios.map((scenario) => { const row = scenario.transition_years.find((candidate) => candidate.year === 'mature'); return `| ${scenario.adoption_percent.toFixed(0)}% | ${scenario.external_input_condition} | ${row.participating_households.toFixed(0)} | ${row.participating_population_people.toFixed(0)} | ${row.productive_land_required_ha.toFixed(1)} | ${row.market_food_demand_displaced_gj_year.toFixed(0)} | ${row.mature_exportable_surplus_gj_year.toFixed(0)} |`; }),
    '',
    '## Caveats',
